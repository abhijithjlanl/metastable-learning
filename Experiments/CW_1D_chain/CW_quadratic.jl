...
for (J,h) in Jh_list
    ...
    for ns  in ns_list
      ...
      δ = 2/ns
      global Ψ(m) = 0.5*a*(m-m_0)^2
      H(m) = ns * (Ψ(m) - S(m))
      S(m) = bin_entropy(0.5*(1+m))
      global Z = sum(exp(ns*Ψ(m)) for m in -1:δ:1)
...
Pl = plot(show=true, xscale=:log10, yscale=:log10, xlabel=L"Number of spins $(n)$", ylabel=L"Strong metastability $(\eta)$",
    title=L"Quadratic free-energy $\Phi^{(2)}$",xticks = xtick)
...
plot!(Pl, x -> 0.2 / x, ns_list[1], ns_list[end], linestyle=:dash, label=L"O(1/n)",legend_position=:bottomleft)
...
savefig("quadratic_metastable_cw.pdf")
